...
/* The Notes index, shared by /blog/ and the per-project filter routes so every page is static. */
export function NotesView({ filter }: { filter: Filter }) {
      ...
      <div className="notes__layout">
        ...
        <aside className="notes__side">
          ...
          <div className="notes__projects">
            {projects.map((p) => (
              <Link key={p.slug} className={`project-row ${filter === p.slug ? "project-row--active" : ""}`} href={filter === p.slug ? "/blog/" : filterHref(p.slug)} aria-pressed={filter === p.slug}>
